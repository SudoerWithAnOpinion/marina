...
const adapter = (): AdapterFunction<SessionAdapter> => 
(LuciaErrorConstructor: LuciaErrorConstructor) => {
  return {
    ...
    setSession: async (session: SessionSchema):  Promise<void> => {
      if (typeof session.active_expires === 'bigint') session.active_expires = Number(session.active_expires);
      if (typeof session.idle_expires === 'bigint') session.idle_expires = Number(session.idle_expires);
